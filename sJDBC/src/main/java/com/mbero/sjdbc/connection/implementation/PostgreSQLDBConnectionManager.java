...
public class PostgreSQLDBConnectionManager implements DBConnectionManager {
	...
	public Connection createConnection(
		...
		if (conn == null) {
			final String dbUrl = "jdbc:postgresql://"
					+ dbConnectionConfiguraton.getHostAndDatabaseName();
			...
			try {
				...
				log.debug("Wystąpił wyjątek w trakcie zestawiania połączenia z bazą : "
						+ dbConnectionConfiguraton.getDatabaseType());
				log.debug(sqlException.getCause(), sqlException);
			} catch (ClassNotFoundException classNotFoundException) {
				log.debug(classNotFoundException.getCause(),
